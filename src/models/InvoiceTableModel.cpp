...
{
    if (!invoices || !index.isValid() || role != Qt::DisplayRole) return QVariant();

    const auto& inv = invoices->at(index.row());
    switch (index.column())
    {
    case 0: return QString::fromStdString(inv.getClient());
    case 1: return inv.getAmount();
    case 2: return inv.isPaid() ? "Yes" : "No";
